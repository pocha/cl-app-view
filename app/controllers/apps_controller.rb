	...
	def new_layout
		...
		if !params[:module_number].nil? and !params[:lesson_number].nil?
			...
			if !params[:task_number].nil?
				@current_task = @current_lesson.tasks.find_by_number(params[:task_number]) || not_found

				@module_no = params[:module_number]
				@lesson_no = params[:lesson_number]
				@task_no = params[:task_number]
				@title = @current_task.name.gsub("-", " ") + ' | Ruby on Rails Tutorial | Codelearn'
				render 'new_layout_task', :layout => 'new_layout'  and return
			else
